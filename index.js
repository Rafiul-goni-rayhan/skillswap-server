...
const verifyToken = async (req, res, next) => {
  ...
  return res
    ...
    .json({
      success: false,
      message: "Unauthorized access. Token or Session missing.",
    });
};
  ...
  try {
    ...
    app.post("/api/reviews", async (req, res) => {
      try {
        ...
        if (freelancer) {
          ...
          console.log(
            `💾 [Synced] History logged & profile updated for ${reviewee_email}`,
          );
        }
    ...
    app.post("/api/create-checkout-session", async (req, res) => {
      try {
        ...
        const finalTitle =
          taskTitle || "SkillSwap Venture Milestone Deployment";
