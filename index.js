...
// CORS কনফিগারেশন - credentials: true রাখা হয়েছে কুকি আদান-প্রদানের জন্য
app.use(
  cors({
    credentials: true,
...
async function run() {
  ...
  // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // ৪. নতুন টাস্ক বা জব পোস্ট করার এপিআই
    // -------------------------------------------------------------------------
      ...
      try {
        if (req.user.role !== "client") {
          return res
            ...
            .json({
              success: false,
              message: "Forbidden. Only clients can post tasks.",
            });
        }
    ...
    // -------------------------------------------------------------------------
    // ৫. Browse Tasks API: Pagination + Search + Category
    // -------------------------------------------------------------------------
    ...
    app.post("/api/reviews", async (req, res) => {
      try {
        ...
        if (!reviewee_email || !rating) {
          return res.status(400).json({ success: false, message: "Reviewee email and rating value are required." });
        }
        ...
        const query = { email: { $regex: `^${reviewee_email.trim()}$`, $options: "i" } };
        ...
        if (freelancer) {
          ...
          console.log(`💾 [Collection Created & Synced] History logged in reviews & profile updated for ${reviewee_email}`);
        }
    ...
    app.get("/api/freelancer/proposals", verifyToken, async (req, res) => {
      try {
        if (req.user.role !== "freelancer") {
          return res.status(403).json({ success: false, message: "Unauthorized. Freelancer role required." });
        }
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    if (user.isBlocked) {
      return res.status(403).json({ 
        success: false, 
        message: "Access Denied: This account has been blocked by the Supreme Admin!" 
      });
    }
...
app.patch("/api/admin/users/:id/block", async (req, res) => {
  try {
    ...
    return res.status(200).json({ 
      success: true, 
      message: blockStatus ? "User successfully blocked." : "User successfully reactivated." 
    });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
...
app.delete("/api/admin/tasks/:id", async (req, res) => {
  try {
    ...
    if (result.deletedCount === 0) {
      return res.status(404).json({ success: false, message: "Task row not found or already purged!" });
    }
    
    return res.status(200).json({ 
      success: true, 
      message: "Task row successfully purged from platform due to safety guidelines violation." 
    });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
...
app.get("/api/admin/transactions", async (req, res) => {
  try {
    ...
    const transactions = await paymentsCollection.find().sort({ created_at: -1 }).toArray();
    ...
    app.post("/api/create-checkout-session", async (req, res) => {
      try {
        ...
        if (!finalAmount || finalAmount <= 0) {
          return res.status(400).json({ 
            success: false, 
            message: "Backend Rejected: Missing required checkout parameters or invalid amount size." 
          });
        }
    ...
    app.post("/api/confirm-session", verifyToken, async (req, res) => {
      try {
        ...
        if (!proposal) {
          return res.status(404).json({ success: false, message: "Proposal record not found." });
        }
        ...
        try {
          ...
          await tasksCollection.updateOne(
            { _id: taskQueryId },
            { $set: { status: "ongoing", hired_freelancer: proposal.freelancer_email } }
          );
        } catch (taskErr) {
          await tasksCollection.updateOne(
        ...
        return res.status(200).json({
          ...
          data: {
            taskTitle: proposal.taskTitle || proposal.title || "Marketplace Task",
        ...
        return res.status(500).json({ success: false, message: "Verification processing crash." });
      }
    });
...
app.get("/api/users", async (req, res) => {
  try {
    ...
    app.post("/api/payments", async (req, res) => {
      try {
        const { client_email, freelancer_email, task_id, amount, transaction_id } = req.body;
        ...
        return res.status(201).json({ success: true, message: "Payment Vector Saved", result });
      } catch (err) {
        return res.status(500).json({ success: false, message: err.message });
      }
    });
    ...
    app.post("/api/reviews", async (req, res) => {
      try {
        ...
        return res.status(201).json({ success: true, message: "Review Core Logged", result });
      } catch (err) {
        return res.status(500).json({ success: false, message: err.message });
      }
    });
